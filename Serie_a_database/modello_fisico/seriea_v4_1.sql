...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `join_squadra_partita_BEFORE_UPDATE` BEFORE UPDATE ON `join_squadra_partita` FOR EACH ROW BEGIN
    ...
    IF (num_partite = 1 AND new.casa_trasferta != 'C' AND new.casa_trasferta !='T') THEN
        SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'ERRORE! Non puoi aggiornare il campo casa_trasferta con un carattere diverso da C o T.';
	ELSEIF (num_partite = 2) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'ERRORE! Impossibile effettuare l''update di un una partita quando sono presenti nel db due record con lo stesso id.';
	END IF;
...
--
-- Dumping events for database 'serie_a'
--
...
-- Dump completed on 2022-04-15 11:43:04
