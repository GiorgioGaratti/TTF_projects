...
CREATE TABLE `tbl_firma` (
  ...
  KEY `IDX_INS` (`data_inserimento` DESC) /*!80000 INVISIBLE */,
  KEY `IDX_MOD` (`data_modifica` DESC),
  KEY `FK_STUDENTE_FIRMA_idx` (`codice_fiscale_studente`),
  KEY `FK_GIORNO_PRESENZA_FIRMA_idx` (`numero_corso`,`data`),
  CONSTRAINT `FK_GIORNO_PRESENZA_FIRMA` FOREIGN KEY (`numero_corso`, `data`) REFERENCES `tbl_giorno_presenza` (`numero_corso`, `data`) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE `tbl_giorno_presenza` (
  ...
  `data_inserimento` timestamp NOT NULL,
  `data_modifica` timestamp NOT NULL,
  PRIMARY KEY (`numero_corso`,`data`),
  KEY `IDX_INS` (`data_inserimento` DESC) /*!80000 INVISIBLE */,
  KEY `IDX_MOD` (`data_modifica` DESC),
  KEY `FK_REGISTRO_GIORNO_PRESENZA_idx` (`numero_corso`,`anno`),
...
CREATE TABLE `tbl_lezione` (
  ...
  KEY `FK_GIORNO_PRESENZA_LEZIONE_idx` (`numero_corso`,`data`),
  KEY `FK_DOCENTE_LEZIONE_idx` (`codice_fiscale_docente`),
  ...
  CONSTRAINT `FK_GIORNO_PRESENZA_LEZIONE` FOREIGN KEY (`numero_corso`, `data`) REFERENCES `tbl_giorno_presenza` (`numero_corso`, `data`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2022-03-16 13:16:01
